...
__mcachefs_fill_stat(struct stat* st, int type, off_t size)
{
  ...
  st->st_mode = type | ((type == S_IFDIR) ? (mcachefs_target_stat.st_mode
      & 0700) : (mcachefs_target_stat.st_mode & 0600));
...
mcachefs_symlink(const char *path, const char *to)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_symlink, to, path, 0, 0, 0, 0, 0,
      NULL);
...
mcachefs_mknod(const char *path, mode_t mode, dev_t rdev)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_mknod, path, NULL, mode, rdev,
      fuse_get_context()->uid, fuse_get_context()->gid, 0, NULL);
...
mcachefs_unlink(const char *path)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_unlink, path, NULL, 0, 0, 0, 0,
      0, NULL);
  return 0;
}
...
mcachefs_rmdir(const char *path)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_rmdir, path, NULL, 0, 0, 0, 0, 0,
      NULL);
...
mcachefs_rename(const char *path, const char *to)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_rename, path, to, 0, 0, 0, 0, 0,
      NULL);
...
mcachefs_chmod(const char *path, mode_t mode)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_chmod, path, NULL, mode, 0, 0, 0,
      0, NULL);
...
mcachefs_truncate(const char *path, off_t size)
{
   ...
   */
  mcachefs_journal_append(mcachefs_journal_op_truncate, path, NULL, 0, 0, 0, 0,
      size, NULL);
...
mcachefs_utime(const char *path, struct utimbuf *buf)
{
  ...
  mcachefs_journal_append(mcachefs_journal_op_utime, path, NULL, 0, 0, 0, 0, 0,
      buf);
    ...
    struct fuse_file_info* info)
{
  ...
  Log("mcachefs_read(path = %s, buf = ..., size = %llu, offset = %llu, fh=%llx)\n", path,
      (unsigned long long) size, (unsigned long long) offset, (unsigned long long) info->fh);
...
#if 0
static int mcachefs_statfs(const char * path, struct statvfs * fsinfo)
  {
    (void) path;
    (void) fsinfo;
    return -ENOSYS;
...
struct fuse_operations mcachefs_oper =
  { .getattr = mcachefs_getattr, .readlink = mcachefs_readlink, .getdir = NULL,
      .mknod = mcachefs_mknod, .mkdir = mcachefs_mkdir,
      .unlink = mcachefs_unlink, .rmdir = mcachefs_rmdir,
      .symlink = mcachefs_symlink, .rename = mcachefs_rename,
      .link = mcachefs_link, .chmod = mcachefs_chmod, .chown = mcachefs_chown,
      .truncate = mcachefs_truncate, .utime = mcachefs_utime,
      .open = mcachefs_open, .read = mcachefs_read, .write = mcachefs_write,
      .statfs = NULL, .flush = mcachefs_flush, .release = mcachefs_release,
      .fsync = mcachefs_fsync, .setxattr = NULL, .getxattr = NULL,
      .listxattr = NULL, .opendir = NULL, .readdir = mcachefs_readdir,
      .fsyncdir = NULL, .init = mcachefs_init, .destroy = mcachefs_destroy,
      .access = NULL, .create = NULL, .ftruncate = NULL, .fgetattr = NULL,
      .lock = NULL, .utimens = NULL, .bmap = NULL,
    };
