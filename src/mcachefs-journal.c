...
{
  struct stat st;
  mcachefs_journal_fsync_path = mcachefs_makepath(".fsync", mcachefs_journal);
...
mcachefs_journal_append(mcachefs_journal_op op, const char* path,
    const char* to, mode_t mode, dev_t rdev, uid_t uid, gid_t gid, off_t size,
    struct utimbuf* utimbuf)
{
  ...
  Log ( "New journal entry : op=%x : path='%s', to='%s' : "
      ...
      entry.op, path, to,
      (long) entry.mode, (long) entry.rdev,
      (long) entry.uid, (long) entry.gid,
  ...
  if (op == mcachefs_journal_op_rename)
    {
      res = stat(mcachefs_journal, &journal_stat);
  ...
  fd = open(mcachefs_journal, O_RDWR | O_CREAT | O_APPEND, 0644);

  if (fd == -1)
    {
      Err ( "Could not open journal '%s' : err=%d:%s\n", mcachefs_journal, errno, strerror(errno) );
      exit(-1);
    }
...
mcachefs_journal_rebuild(const char* rename_path, const char* rename_to)
{
  ...
  fd_src = open(mcachefs_journal, O_RDONLY);
  if (fd_src == -1)
    {
  ...
  journal_tgt = (char*) malloc(strlen(mcachefs_journal) + strlen(
      mcachefs_journal_suffix) + 1);
  strcpy(journal_tgt, mcachefs_journal);
  ...
  while (1)
    {
      ...
      if (entry.op == mcachefs_journal_op_fsync && strncmp(path, rename_path,
          rename_path_sz) == 0)
        {
          strncpy(altered_path_suffix, &(path[rename_path_sz]), PATH_MAX
              - rename_to_sz);
  ...
  if (unlink(mcachefs_journal))
    {
      Err ( "Could not unlink : err=%d:%s\n", errno, strerror(errno) );
    }
  if (rename(journal_tgt, mcachefs_journal))
    {
      Err ( "Could not rename %s to %s : err=%d:%s\n", journal_tgt, mcachefs_journal, errno, strerror(errno) );
      res = -errno;
    }
...
mcachefs_journal_was_renamed(const char* newpath)
{
  ...
  mcachefs_journal_lock ();
  fd = open(mcachefs_journal, O_RDONLY);
    ...
    const char* path, const char* to)
{
  Log ( "Apply journal entry : op=%s (%x) : path='%s', to='%s' : "
      "mode=%lo, dev=%ld, uid=%ld, gid=%ld, size=%lu, utimebuf=%lu,%lu\n",
      entry->op < mcachefs_journal_op_MAX ? mcachefs_journal_op_label[entry->op] : "INVALID",
      entry->op,
      path, to,
      (long) entry->mode, (long) entry->rdev,
      (long) entry->uid, (long) entry->gid,
...
mcachefs_journal_apply()
{
   ...
   */
  if (mcachefs_journal_apply_updates(mcachefs_journal))
    {
   ...
   */
  res = rename(mcachefs_journal, mcachefs_journal_fsync_path);
  if (res == -1)
    {
      Bug ( "Could not rename '%s' to '%s' : err=%d:%s\n", mcachefs_journal, mcachefs_journal_fsync_path, errno, strerror(errno) );
    }
...
mcachefs_journal_drop()
{
  ...
  if (truncate(mcachefs_journal, 0))
    {
  ...
  while (1)
    {
      ...
      __VOPS_WRITE ( mvops, "[%lu] %s (%x) : path='%s', to='%s' : "
          "mode=%lo, dev=%ld, uid=%ld, gid=%ld, size=%lu, utimebuf=%lu,%lu\n",
          entry_nb,
          entry.op < mcachefs_journal_op_MAX ? mcachefs_journal_op_label[entry.op] : "INVALID",
          entry.op,
