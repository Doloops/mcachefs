...
{
    struct stat st;
    mcachefs_journal_fsync_path =
        mcachefs_makepath (".fsync", mcachefs_journal);
...
mcachefs_journal_rebuild (const char *rename_path, const char *rename_to)
{
    ...
    journal_tgt =
        (char *) malloc (strlen (mcachefs_journal) +
                              ...
                              const char *path, const char *to)
{
    Log ("Apply journal entry : op=%s (%x) : path='%s', to='%s' : "
         "mode=%lo, dev=%ld, uid=%ld, gid=%ld, size=%lu, utimebuf=%lu,%lu\n",
         entry->op <
         mcachefs_journal_op_MAX ? mcachefs_journal_op_label[entry->
                                                             op] : "INVALID",
