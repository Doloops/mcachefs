...
mcachefs_metadata_format()
{
  ...
  pwrite(mcachefs_metadata_fd, &mhead, sizeof(struct mcachefs_metadata_head_t),
      0);
  ...
  if (stat(mcachefs_metafile, &st) == 0 && st.st_size)
    {
      ...
      if ( mcachefs_metadata_fd == -1 )
        {
          Err ( "Could not open '%s' : err=%d:%s\n", mcachefs_metafile, errno, strerror(errno));
        }
      Log ( "Openned metafile '%s' at fd=%d\n", mcachefs_metafile, mcachefs_metadata_fd );
...
mcachefs_metadata_extend()
{
  ...
  mcachefs_metadata = mmap(mcachefs_metadata, mcachefs_metadata_size, PROT_READ
      | PROT_WRITE, MAP_SHARED, mcachefs_metadata_fd, 0);
...
mcachefs_metadata_remove_hash(struct mcachefs_metadata_t* mdata)
{
  ...
  if (!mdata->left || !mdata->right)
    {
      child = mcachefs_metadata_get(mdata->left ? mdata->left : mdata->right);
      child->up = up->id;
      if (iamleft)
        up->left = child->id;
      else
  ...
  for (child = left; child->right; child = mcachefs_metadata_get(child->right))
    ;
...
mcachefs_metadata_unlink_entry(struct mcachefs_metadata_t* mdata)
{
  ...
  Bug ( "Child '%s' (%llu) is not child of father '%s' (%llu)\n",
      mdata->d_name, mdata->id,
      father->d_name, mdata->id );
}
...
mcachefs_metadata_get_child(struct mcachefs_metadata_t* father)
{
  ...
  else if (father->father == mcachefs_metadata_id_root && strncmp(
      father->d_name, ".mcachefs", NAME_MAX + 1) == 0)
    {
...
mcachefs_metadata_getattr(const char* path, struct stat* stbuf)
{
  ...
  Log ( "dev=%ld, ino=%ld, mode=%lo, link=%lx, uid=%ld, gid=%ld, rdev=%ld, size=%lu, blksz=%ld, blkcnt=%ld, at=%ld, mt=%ld, ct=%ld\n",
      (long) mdata->st.st_dev, (long) mdata->st.st_ino, (long) mdata->st.st_mode, (long) mdata->st.st_nlink,
      (long) mdata->st.st_uid, (long) mdata->st.st_gid,
      (long) mdata->st.st_rdev, (long) mdata->st.st_size, (long) mdata->st.st_blksize, (long) mdata->st.st_blocks,
...
mcachefs_metadata_get_path(struct mcachefs_metadata_t* mdata)
{
  ...
  for (mcurrent = mdata; mcurrent->father; mcurrent = mcachefs_metadata_get(
      mcurrent->father))
    {
...
mcachefs_metadata_schedule_fill_child_entries(struct mcachefs_metadata_t* mdata)
{
  ...
  for (child = mcachefs_metadata_get_child(mdata); child; child
      = mcachefs_metadata_get(child->next))
    {
      if (S_ISDIR ( child->st.st_mode ))
        {
          if (mdata->id == mcachefs_metadata_id_root && strcmp(child->d_name,
              ".mcachefs") == 0)
            {
      ...
      if (oldstat->st_size != newstat->st_size)
        {
          Info ( "Update %s/%s : Sizes differ (old=%lu, new=%lu)!\n",
              path, d_name,
              (unsigned long) oldstat->st_size, (unsigned long) newstat->st_size );
    ...
    struct mcachefs_metadata_t* mdata, int depth)
{
  ...
  __VOPS_WRITE ( mvops, "%s[%llu] h=%llx : '%s' (c=%llu,n=%llu,f=%llu, ulr=%llu:%llu:%llu), fh=%lx\n",
      dspace, mdata->id, mdata->hash, mdata->d_name,
      mdata->child, mdata->next, mdata->father,
      mdata->up, mdata->left, mdata->right,
      (unsigned long) mdata->fh );

  if (mdata->child && mdata->child != mcachefs_metadata_id_EMPTY)
    mcachefs_file_dump_meta(mvops, mcachefs_metadata_get(mdata->child), depth
        + 1);

  if (mdata->next)
    mcachefs_file_dump_meta(mvops, mcachefs_metadata_get(mdata->next), depth);
}
    ...
    struct mcachefs_metadata_t* mdata, int depth, hash_t min, hash_t max)
{
  ...
  __VOPS_WRITE ( mvops, "%s[%llu], hash=%llx : '%s' (region [%llx:%llx]), ulr=%llu/%llu/%llu, coll=n:%llu/p:%llu\n",
      dspace, mdata->id, mdata->hash, mdata->d_name,
      min, max, mdata->up, mdata->left, mdata->right,
  ...
  if (mdata->left)
    mcachefs_file_dump_hash(mvops, mcachefs_metadata_get(mdata->left), depth
        + 1, min, mdata->hash - 1);
  if (mdata->right)
    mcachefs_file_dump_hash(mvops, mcachefs_metadata_get(mdata->right), depth
        + 1, mdata->hash, max);
  if (mdata->collision_next)
    mcachefs_file_dump_hash(mvops,
        mcachefs_metadata_get(mdata->collision_next), depth, mdata->hash - 1,
        mdata->hash + 1);
}
...
mcachefs_metadata_dump(struct mcachefs_file_t* mvops)
{
  ...
  __VOPS_WRITE ( mvops, "--------------- Metadata hash tree root=%s -----------------\n",
      mcachefs_metadata_get_root()->d_name );
