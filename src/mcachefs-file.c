...
void mcachefs_file_source_init(struct mcachefs_file_source_t* source)
{
  source->use = 0;
  source->fd = -1;
  source->wr = 0;
 ...
 */
int mcachefs_file_may_reuse_source(struct mcachefs_file_t* mfile, struct mcachefs_file_source_t* source,
    int asked_wr )
{
  if (source->fd != -1)
    {
    ...
    char* (*path_translator)(const char* path))
{
  ...
  Log ( "OPEN path='%s', translated_path='%s' => fd=%d, flags=%lo, mode=%lo, use=%d, wr=%d, asked=%d\n",
      mfile->path, translated_path, source->fd, (long) flags, (long) mode,
      source->use, source->wr, asked_wr );
  ...
  if (real)
    {
      ...
      fd = mcachefs_file_do_open(mfile, flags, mode, &(mfile->sources[MCACHEFS_FILE_SOURCE_REAL]),
    ...
    {
      if (mode == O_RDONLY && mfile->backing_status == MCACHEFS_FILE_BACKING_ASKED)
        {
      ...
      fd = mcachefs_file_do_open(mfile, flags, mode, &(mfile->sources[MCACHEFS_FILE_SOURCE_BACKING]),
