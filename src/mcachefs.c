...
int
config_getnbthreads(config_state *cfg,const char* mountpoint, const char* value)
{
  ...
  if ( val == NULL ) return mcachefs_transfer_default_threads_nb;
  ival = atoi(val);
...
main(int argc, char *argv[])
{
  ...
  if (!mcachefs_backing)
    {
      fprintf(stderr, "No 'backing' value for mount point '%s'\n",
          mcachefs_mountpoint);
      exit(-1);
    }
  ...
  if (!mcachefs_metafile)
    {
      fprintf(stderr, "No 'metafile' value for mount point '%s'\n",
          mcachefs_mountpoint);
      exit(-1);
    }
  ...
  if (!mcachefs_journal)
    {
      fprintf(stderr, "No 'journal' value for mount point '%s'\n",
          mcachefs_mountpoint);
      exit(-1);
    }
  ...
  mcachefs_transfer_default_threads_nb = config_getnbthreads(cfg, mcachefs_mountpoint, "threads");
