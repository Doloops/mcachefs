...
void mcachefs_file_timeslice_insert_in_ts ( struct mcachefs_file_t* mfile, int timeslice )
{
    /**
    ...
    Log ( "post-insert : mfile=%p, timeslice=%d, head=%p, next=%p, next->previous=%p (next->next=%p)\n",
        mfile, mfile->timeslice, mcachefs_file_timeslices[mfile->timeslice], mfile->timeslice_next,
        mfile->timeslice_next ? mfile->timeslice_next->timeslice_previous : NULL,
        mfile->timeslice_next ? mfile->timeslice_next->timeslice_next : NULL );
        
    if ( mfile->timeslice_next && mfile->timeslice_next->timeslice_next 
          && mfile->timeslice_next->timeslice_next->timeslice_previous != mfile->timeslice_next )
      {
         Bug ( "." );
      }
}
...
void mcachefs_file_timeslice_remove (  struct mcachefs_file_t* mfile )
{
    ...
    Log ( "post-remove : mfile=%p, prev=%p, prev->next=%p, next=%p, next->prev=%p\n",
        mfile, mfile->timeslice_previous, mfile->timeslice_previous ? mfile->timeslice_previous->timeslice_next : NULL,
        mfile->timeslice_next, mfile->timeslice_next ? mfile->timeslice_next->timeslice_previous : NULL );
...
void mcachefs_file_timeslice_cleanup_list ( int age, void (*action) ( struct mcachefs_file_t* mfile ) )
{
...
void mcachefs_file_timeslice_cleanup_freed ()
{
...
void mcachefs_file_timeslice_cleanup ( )
{
     ...
     * Cleanup files
     */
    mcachefs_file_timeslice_cleanup_list ( mcachefs_get_file_ttl(), &mcachefs_file_cleanup_file );   
     ...
     * Cleanup dirs
     */
    mcachefs_file_timeslice_cleanup_list ( mcachefs_get_metadata_ttl(), &mcachefs_metadata_cleanup_dir );   
#endif
...
void mcachefs_file_timeslices_dump_source ( struct mcachefs_file_t* mvops, struct mcachefs_file_source_t* source, const char* label )
{
      ...
      if ( source->nbrd )
        {
          __VOPS_WRITE ( mvops, " (read #%lu : %luk)",
            (unsigned long) source->nbrd, (unsigned long) source->bytesrd >> 10 );
        }
      if ( source->nbwr )
        {
          __VOPS_WRITE ( mvops, " (write #%lu : %luk)",
            (unsigned long) source->nbwr, (unsigned long) source->byteswr >> 10 );
        }
    }
}

void mcachefs_file_timeslices_dump_ts ( struct mcachefs_file_t* mvops, struct mcachefs_file_t* mhead )
{
    char ctype;
    ...
    for ( mfile = mhead ; mfile ; mfile = mfile->timeslice_next )
      {
#if 0
        __VOPS_WRITE ( mvops, "\t%p:%s, type=%d, ts=%d, hash=%llx, prev=%p, next=%p, metaid=%lu",
            mfile, mfile->path, mfile->type, mfile->timeslice, mfile->hash,
            mfile->timeslice_previous, mfile->timeslice_next,
            (unsigned long) mfile->metadata_id );
#endif
        if ( mfile->type > 3 ) ctype = '?';
        else ctype = ctypes[mfile->type];
        __VOPS_WRITE ( mvops, "\t%s %c,use=%d", mfile->path, ctype, mfile->use );
        if ( mfile->type == mcachefs_file_type_file )
          {
            mcachefs_file_timeslices_dump_source(mvops, &(mfile->sources[MCACHEFS_FILE_SOURCE_BACKING]), "back");
            mcachefs_file_timeslices_dump_source(mvops, &(mfile->sources[MCACHEFS_FILE_SOURCE_REAL]), "real");
        ...
        else if ( mfile->type == mcachefs_file_type_vops )
          {
            __VOPS_WRITE ( mvops, ", contents=%p, size=%lu, alloced=%lu",    
                mfile->contents,
                (unsigned long) mfile->contents_size, (unsigned long) mfile->contents_alloced );
          }
...
void mcachefs_file_timeslices_dump ( struct mcachefs_file_t* mvops )
{
    ...
    for ( j = 0 ; j < mcachefs_file_timeslice_nb + 2 ; j++ )
      {
        if ( ! mcachefs_file_timeslices[j] ) continue;
        __VOPS_WRITE ( mvops, "At timeslice %d : head=%p%s%s%s\n", j, mcachefs_file_timeslices[j],
                j == mcachefs_file_timeslice_current ? " => current" : "",
                j == mcachefs_file_timeslice_garbage ? " => garbage" : "",
