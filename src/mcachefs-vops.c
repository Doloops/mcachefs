...
const char* state_names[] = { "normal", "full", "handsup", "nocache", "quitting", NULL };
...
void mcachefs_setstate ( int state ) { mcachefs_state = state; }
int mcachefs_getstate ( )            { return mcachefs_state;  }

void mcachefs_setwrstate ( int wrstate ) { mcachefs_wrstate = wrstate; }
int mcachefs_getwrstate ( )              { return mcachefs_wrstate; }

int mcachefs_get_file_thread_interval()  { return mcachefs_file_thread_interval; }

int mcachefs_get_file_ttl() { return mcachefs_file_ttl; }
int mcachefs_get_metadata_ttl() { return mcachefs_metadata_ttl; }

off_t mcachefs_get_transfer_max_rate () { return mcachefs_transfer_max_rate; }

int mcachefs_get_cleanup_backing_age() { return mcachefs_cleanup_backing_age; }
const char* mcachefs_get_cleanup_backing_prefix()
{
  if ( mcachefs_cleanup_backing_prefix ) return mcachefs_cleanup_backing_prefix;
  return "/";
}
...
void mcachefs_vops_build ( struct mcachefs_file_t* mvops )
{
    if ( mvops->contents )
      {
   ...
   if ( strcmp ( file, state_file ) == 0 )
      ...
      {
        __VOPS_WRITE ( mvops, "%lu", (unsigned long) mcachefs_transfer_max_rate );
      }
      ...
      {
        __VOPS_WRITE ( mvops, "%lu", (unsigned long) mcachefs_get_cleanup_backing_age() );
      }
    else
      {
...
int mcachefs_vops_parse_int ( struct mcachefs_file_t* mvops, const char* values[] )
{
    int iter;
    off_t t;
...
void mcachefs_vops_parse ( struct mcachefs_file_t* mvops )
{
    const char* file = &(mvops->path[11]);
    char* c;
    int result;
   ...
   if ( strcmp ( file, state_file ) == 0 )
    ...
    else if ( strcmp ( file, "cleanup_backing_prefix" ) == 0 )
      {
        ...
        mcachefs_cleanup_backing_prefix = (char*) malloc ( mvops->contents_size + 2 );
        memcpy ( mcachefs_cleanup_backing_prefix, mvops->contents, mvops->contents_size );
...
void mcachefs_vops_cleanup_vops ( struct mcachefs_file_t* mvops )
{
     if ( mvops->contents )
        {
