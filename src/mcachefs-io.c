...
int
mcachefs_open_mfile(struct mcachefs_file_t *mfile, struct fuse_file_info *info, mcachefs_file_type_t type, struct stat *metadata_st)
{
    if (type == mcachefs_file_type_file)
    {
        ...
        info->direct_io = 1;
#endif
        if (mcachefs_config_get_read_state() != MCACHEFS_STATE_NOCACHE || __IS_WRITE(info->flags))
        {
    ...
    {
        Log("VOPS OPEN %s : fh=%llu, contents=%p\n", mfile->path, (unsigned long long) info->fh, mfile->contents);
        info->direct_io = 1;
...
int
mcachefs_read_vops(struct mcachefs_file_t *mfile, char *buf, size_t size, off_t offset)
{
    int res = 0;
    ...
    if (offset >= mfile->contents_size)
    {
        Log("VOPS : Exceeding size for '%s' : offset=%llu, mfile->contents_size=%llu !\n", mfile->path,
            (unsigned long long) offset, (unsigned long long) mfile->contents_size);
        return 0;
    }
    res = ((off_t) size <= mfile->contents_size - offset) ? (int) size : (int) (mfile->contents_size - offset);
...
mcachefs_read_wait_accessible(struct mcachefs_file_t *mfile, size_t size, off_t offset)
{
    ...
    while (1)
    {
        ...
        if (mfile->cache_status == MCACHEFS_FILE_BACKING_NONE
            || mfile->cache_status == MCACHEFS_FILE_BACKING_ASKED || mfile->cache_status == MCACHEFS_FILE_BACKING_ERROR)
        {
            use_real = 1;
            break;
        }
        if (mfile->cache_status == MCACHEFS_FILE_BACKING_IN_PROGRESS && mfile->transfer.transfered_size >= offset + (off_t) size)
        {
            use_real = 0;
            break;
        }
        ...
        Log("Waiting for end of backing for file '%s', offset=%luk, size=%luk, end of segment=%luk\n", mfile->path,
            (unsigned long) offset >> 10, (unsigned long) size >> 10, (unsigned long) (offset + (off_t) size) >> 10);
        mcachefs_file_lock_file(mfile);
        waited_backing++;
    }
    mcachefs_file_unlock_file(mfile);
...
int
mcachefs_read_file(struct mcachefs_file_t *mfile, char *buf, size_t size, off_t offset)
{
    int res = 0;
    int fd;
    int use_real;
...
int
mcachefs_write_vops(struct mcachefs_file_t *mfile, const char *buf, size_t size, off_t offset)
{
    mcachefs_file_lock_file(mfile);
    if (offset + (off_t) size > mfile->contents_alloced)
    {
        mfile->contents_alloced = offset + size;
        mfile->contents = (char *) realloc(mfile->contents, mfile->contents_alloced);
    ...
    Log("VOPS WRITE '%s' : write %llu, now alloced=%llu, size=%llu\n",
        mfile->path, (unsigned long long) size, (unsigned long long) mfile->contents_alloced, (unsigned long long) mfile->contents_size);
...
int
mcachefs_write_file(struct mcachefs_file_t *mfile, const char *buf, size_t size, off_t offset)
{
    ssize_t bytes;
    int fd;
    int use_real = 0;
    ...
    if (!mfile->dirty)
    {
        ...
        if (!mfile->dirty)
        {
            ...
            mfile->dirty = 1;
            mcachefs_file_unlock_file(mfile);
            mcachefs_journal_append(mcachefs_journal_op_fsync, mfile->path, NULL, 0, 0, 0, 0, 0, NULL);
        }
        else
        {
            mcachefs_file_unlock_file(mfile);
...
{
    mcachefs_file_lock_file(mfile);
    if (mfile->sources[MCACHEFS_FILE_SOURCE_BACKING].fd != -1 && mfile->sources[MCACHEFS_FILE_SOURCE_BACKING].wr)
    {
        if (fsync(mfile->sources[MCACHEFS_FILE_SOURCE_BACKING].fd))
        {
            Err("Could not fsync(%s) : err=%d:%s\n", mfile->path, errno, strerror(errno));
        }
    }
    mcachefs_file_unlock_file(mfile);
    return 0;
...
mcachefs_release_mfile(struct mcachefs_file_t *mfile, struct fuse_file_info *info)
{
    ...
    else if (mfile->type == mcachefs_file_type_file)
    {
        Log("[STATS] '%s' : rd=%lu,%lu,wr=%lu,%lu, real=%lu,%lu\n",
            ...
            (unsigned long) mfile->sources[0].byteswr, (unsigned long) mfile->sources[1].nbrd, (unsigned long) mfile->sources[1].bytesrd);
        if (__IS_WRITE(info->flags))
        {
