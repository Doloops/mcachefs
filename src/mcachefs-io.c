...
int
mcachefs_open_mfile(struct mcachefs_file_t *mfile, struct fuse_file_info *info,
        mcachefs_file_type_t type)
{
    if (type == mcachefs_file_type_file)
    {
        ...
        {
            Log(
                    "MCachefs : skipping backup of file %s, state=%d\n", mfile->path, mcachefs_config_get_read_state ());
        ...
        off_t offset)
{
    ...
    while (1)
    {
        ...
        Log(
                "Waiting for end of backing for file '%s', offset=%luk, size=%luk, end of segment=%luk\n", mfile->path, (unsigned long) offset >> 10, (unsigned long) size >> 10, (unsigned long) (offset + (off_t) size) >> 10);
        mcachefs_file_lock_file(mfile);
        waited_backing++;
    }
    mcachefs_file_unlock_file(mfile);
    Log(
            "Waiting read path='%s' : mfile->backing_status=%d, use_real=%d\n", mfile->path, mfile->cache_status, use_real);
    return use_real;
}
...
int
mcachefs_write_vops(struct mcachefs_file_t *mfile, const char *buf, size_t size,
        off_t offset)
{
    mcachefs_file_lock_file(mfile);
...
int
mcachefs_write_file(struct mcachefs_file_t *mfile, const char *buf, size_t size,
        off_t offset)
{
    ssize_t bytes;
    int fd;
    ...
    Log(
            "write to '%s' ok : written %ld bytes at offset %ld\n", mfile->path, (long) bytes, (long) offset);
    return (int) bytes;
}
        ...
        size_t size, off_t offset)
{
    ...
    Err(
            "Invalid type for write() : %s has type %d\n", mfile->path, mfile->type);
    return -ENOSYS;
}
        ...
        struct fuse_file_info *info)
{
    ...
    {
        Log(
                "[STATS] '%s' : rd=%lu,%lu,wr=%lu,%lu, real=%lu,%lu\n", mfile->path, (unsigned long) mfile->sources[0].nbrd, (unsigned long) mfile->sources[0].bytesrd, (unsigned long) mfile->sources[0].nbwr, (unsigned long) mfile->sources[0].byteswr, (unsigned long) mfile->sources[1].nbrd, (unsigned long) mfile->sources[1].bytesrd);
        if (__IS_WRITE (info->flags))
        {
