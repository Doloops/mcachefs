  ...
  if (type == mcachefs_file_type_file)
    {
      ...
      info->direct_io = 1;
#endif
      if ( mcachefs_getstate() != MCACHEFS_STATE_NOCACHE || __IS_WRITE ( info->flags ) )
        {
      ...
      if (!mdata)
      ...
      else if ((off_t) size + offset > mdata->st.st_size)
        {
          Log ( "Read after tail '%s' : size=%lu, offset=%lu, end=%lu, size=%lu\n",
              mfile->path,
              (unsigned long) size, (unsigned long) offset, (unsigned long) ((off_t) size + offset),
              (unsigned long) mdata->st.st_size );
        }
      else
        {
          Err ( "Could not fully read '%s' : asked=%lu, had %d, offset=%lu, max=%lu, tail=%lu\n",
              mfile->path, (unsigned long) size, res,
              (unsigned long) offset, (unsigned long) (offset+size), (unsigned long) mdata->st.st_size );
        }
      if (mdata)
...
mcachefs_write_vops(struct mcachefs_file_t* mfile, const char* buf, size_t size, off_t offset)
{
  ...
  Log ( "VOPS WRITE '%s' : write %llu, now alloced=%llu, size=%llu\n", mfile->path,
      (unsigned long long) size, (unsigned long long) mfile->contents_alloced, (unsigned long long) mfile->contents_size );
  ...
  if (!mfile->dirty)
    {
      ...
      mfile->dirty = 1;
      mcachefs_file_unlock_file ( mfile );
      mcachefs_journal_append(mcachefs_journal_op_fsync, mfile->path, NULL, 0, 0,
          0, 0, 0, NULL);
    }
  else
    {
  ...
  if (mfile->type == mcachefs_file_type_vops)
  ...
  else if (mfile->type == mcachefs_file_type_file)
    {
      Log ( "[STATS] '%s' : rd=%lu,%lu,wr=%lu,%lu, real=%lu,%lu\n",
          mfile->path,
          (unsigned long)mfile->sources[0].nbrd,(unsigned long)mfile->sources[0].bytesrd,
          (unsigned long)mfile->sources[0].nbwr,(unsigned long)mfile->sources[0].byteswr,
          (unsigned long)mfile->sources[1].nbrd,(unsigned long)mfile->sources[1].bytesrd );
      if (__IS_WRITE ( info->flags ))
        {
