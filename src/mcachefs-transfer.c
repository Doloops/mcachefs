...
mcachefs_transfer_start_threads()
{
  ...
  for ( type = 0 ; type < MCACHEFS_TRANSFER_TYPES ; type++ )
    {
      mcachefs_transfer_threads_nb += mcachefs_transfer_threads_type_nb[type];
    }
  ...
  mcachefs_transfer_threads = (struct mcachefs_transfer_thread_t*)
         malloc(sizeof(struct mcachefs_transfer_thread_t) * mcachefs_transfer_threads_nb);
...
mcachefs_transfer_queue_file(struct mcachefs_file_t* mfile, int type)
{
  ...
  transfer = (struct mcachefs_transfer_queue_t*) malloc(
      sizeof(struct mcachefs_transfer_queue_t));
  ...
  if (stat(realpath, &realstat) == 0)
    {
      ...
      Log ( "Will write back : real mtime=%lu, real size=%lu, metatstat mtime=%lu, metastat size=%lu\n",
          realstat.st_mtime, (unsigned long) realstat.st_size,
          timbuf->modtime, (unsigned long) mfile->transfer.total_size );
    }
...
mcachefs_transfer_file(struct mcachefs_file_t* mfile, int tobacking)
{
  ...
  while (1)
    {
      ...
      if (mcachefs_get_transfer_max_rate() && adjusted_rate
          >= mcachefs_get_transfer_max_rate())
        {
          ...
          penalty.tv_sec = 0;
          Log ( "window_size=%lu, max_rate=%lu\n", (unsigned long) window_size, (unsigned long) mcachefs_get_transfer_max_rate() );
          penalty.tv_nsec = ((window_size << 20)
              / mcachefs_get_transfer_max_rate());
      ...
      Log ( "Transfered %luk of %luk (%lu%%), rate current=%lukb/s, global=%lukb/s, adjusted=%lukb/s, window size=%lu\n",
          ((unsigned long) offset) >> 10, ((unsigned long)size) >> 10,
          (unsigned long) ( offset * 100 / size ),
          (unsigned long) rate, (unsigned long) global_rate, (unsigned long) adjusted_rate,
  ...
  Info ( "End of transfer (%s) for '%s' : %ld usec, copied '%lu', rate=%lu kb/sec\n",
      tobacking ? "backing" : "real", mfile->path, interval,
      (unsigned long) size, (unsigned long) ( (size * 1000) / interval ) );
...
mcachefs_transfer_dump(struct mcachefs_file_t* mvops)
{
  ...
  for (cur = 0; cur < mcachefs_transfer_threads_nb ; cur++)
    {
      ...
      if (mfile->type == mcachefs_file_type_dir)
      ...
      else
        {
          ...
          __VOPS_WRITE ( mvops, "\t%s %s : %luk/%luk (%lu%%), rate=%lukb/s\n",
              (tobacking ? "Backup   " : "Writeback"),
              mfile->path, ((unsigned long) offset) >> 10, ((unsigned long)size) >> 10,
  ...
  if (total_size)
    {
      __VOPS_WRITE ( mvops, "Total transfered : %luk/%luk (%lu%%), rate=%lukb/s\n",
          ((unsigned long) total_transfered) >> 10, ((unsigned long) total_size) >> 10,
  ...
  for (mqueue = mcachefs_transfer_queue_head; mqueue; mqueue = mqueue->next)
    {
      tobacking = (mqueue->mfile->backing_status != MCACHEFS_FILE_BACKING_DONE);
      __VOPS_WRITE ( mvops, "\t%s %s\n",
          (mqueue->mfile->type == mcachefs_file_type_dir) ? "Fill meta" : (tobacking ? "Backup   " : "Writeback"),
          mqueue->mfile->path );
    }
  mcachefs_transfer_unlock ();
}
